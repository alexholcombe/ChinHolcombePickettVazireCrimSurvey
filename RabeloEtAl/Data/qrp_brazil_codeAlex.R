...
### Read data ====
mypath<-file.path("RabeloEtAl","Data")
mypath<-"/Users/alexh/Documents/grants,fellowships,grad school,jobsearch/grants/DP2021_ChinHolcombeVazirePickett/PickettSimineProject/git_repo_ChinHolcombePickettVazireCrimSurvey/RabeloEtAl/Data"
setwd(mypath) # set working directory to this directory
...
## working directory)
# Brazil
#AOH:The below is the old file without column names, making the subset line fail, so I had to comment it out
#df_br <- read.csv("tidy_data.csv", header=TRUE, sep=";") 
...
# US
#LOOKS LIKE THEY PUT THE WRONG FILE ON OSF, BECAUSE THE BELOW IS JUST THE CODEBOOK, NOT THE DATA
...
write.csv(df_it, "QRP_it.csv") # save US database #AOH:BUT THIS IS ITALY!

## US data cleaning
# Excluding demographics and open questions from US data frame and 
# standardizing variable names
#BELOW LINE FAILS BECAUSE NO SUCH COLUMNS BECAUASE THEY UPLOADED THE CODEBOOK, NOT THE DATA
#df_us <- subset(df_us, finished == 1 & bts == 0) #selecting only complete cases from control group


## merging data frames
df_total <- rbind(df_comp, df_it)
...
QRP <- c(#"QRP 1", "QRP 2", "QRP 3", "QRP 4", "QRP 5", "QRP 6", "QRP 7","QRP 8", "QRP 9", "QRP 10", 
  "QRP 1", "QRP 2", "QRP 3", "QRP 4", "QRP 5",
  "QRP 6", "QRP 7", "QRP 8", "QRP 9", "QRP 10", "QRP 1", "QRP 2", "QRP 3",
  "QRP 4", "QRP 5", "QRP 6", "QRP 7", "QRP 8", "QRP 9", "QRP 10")
factor(QRP)

No <- c(#1, 1, 0, 3, 4, 3, 5, 8, 0, 100,
        6,  3, 14, 9, 16, 14, 9, 9, 43, 60,
      6, 4, 6, 4, 14.6, 3, 9, 10, 20, 50) 

Pos <- c(#10, 21, 17, 16, 22, 26, 25, 35, 50, 0,
  60, 42, 72, 70, 47, 66, 82,
...
Yes <- c(#89, 78, 83, 81, 74, 71, 70, 57, 50, 0, 
    34, 55, 14, 22, 37, 21, 9,
...
library(Rmisc)
saveRDS(df_long,"prevalenceLongAfterAlexEditingAndRunningBrazilCode.rds")


##########
#IN THE BELOW, THE DATA IS SUMMARISED SO THE DISTRIBUTION IS LOST
# Generate data frame with CI
bars <- Rmisc::summarySE(df_long, measurevar="val", groupvars=c("QRP","QRP_type"))
